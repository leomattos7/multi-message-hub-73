...
export const doctorProfileService = {
  async getProfileByDoctorId(doctorId: string) {
    try {
      ...
      // Use a direct RPC call to the database to handle non-UUID strings
      const { data, error } = await supabase
  ...
  async createProfile(doctorId: string, profile: any) {
    try {
      ...
      // Insert the profile with text ID type instead of UUID
      // Use a different approach to handle the non-UUID ID
      const { data, error } = await supabase.rpc('create_doctor_profile', {
        p_id: doctorId,
        p_bio: profile.bio,
        ...
        p_profile_image_url: profile.profile_image_url,
  ...
  async updateProfile(doctorId: string, updates: any) {
    try {
      ...
      // Update profile using RPC for non-UUID ID
      const { data, error } = await supabase.rpc('update_doctor_profile', {
        p_id: doctorId,
        p_bio: updates.bio,
        p_specialty: updates.specialty,
        p_name: updates.name,
        p_email: updates.email,
        p_phone: updates.phone,
        p_address: updates.address,
        p_profile_image_url: updates.profile_image_url,
