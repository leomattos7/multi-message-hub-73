...
export const doctorProfileService = {
  ...
  async createProfile(doctorId: string, profile: any) {
    ...
    if (doctorId.includes('user-')) {
      // Create a UUID v4 based on the user ID
      // Fix: Use any type for the RPC call since it's not in the types
      const { data: uuidResult } = await supabase.rpc('generate_uuid_v4', {}, { count: 'exact' }) as unknown as { data: string };
      if (uuidResult) {
