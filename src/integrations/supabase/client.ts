...
export const doctorProfileService = {
  async getProfileByDoctorId(doctorId: string) {
    try {
      ...
      // Query by the raw ID directly without trying to convert it to UUID
      const { data, error } = await supabase
  ...
  async createProfile(doctorId: string, profile: any) {
    try {
      ...
      // Insert the profile with the original ID without any conversion
      const { data, error } = await supabase
        .from('doctor_profiles')
        .insert({
          id: doctorId, // Use the ID as-is
          bio: profile.bio,
