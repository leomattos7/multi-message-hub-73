...
export const doctorProfileService = {
  ...
  async createProfile(doctorId: string, profile: any) {
    ...
    if (doctorId.includes('user-')) {
      // Fix the RPC call type issue by providing an explicit type cast for both parameters and return type
      const { data: uuidResult } = await supabase.rpc('generate_uuid_v4', {} as never) as { data: string };
      if (uuidResult) {
